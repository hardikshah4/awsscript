...
variable "a-mypublicipmask" {
  description = "Access ipchicken.com to have the public IP of your machine, enter it with mask - Example : 1.1.1.1/32"
}

variable "b-firewallname" {
}

variable "c-vpcname" {
}

variable "d-mgmtsubname" {
}

variable "e-untrustsubname" {
}

variable "f-trustsubname" {
}

variable "g-webservname" {
}

variable "h-panoramaname" {
}

variable "i-securitygroup" {
}
...
variable "bucket" {
  ...
  default = {
    VM-300="vmseries-bootstrap-aws-s3bucket=hshahone"
    VM-500="vmseries-bootstrap-aws-s3bucket=hshahone-500"
    VM-700="vmseries-bootstrap-aws-s3bucket=hshahone-700"
  }
}
...
resource "aws_network_interface" "Trust" {
  ...
  depends_on = [aws_instance.awsha_fw1,aws_eip.fw1mgmtpublic,aws_network_interface.Untrust]
}

resource "aws_instance" "awsha_panorama" {
  ami             = var.pano_amiid[var.a-aws_region_v]
  instance_type   = "m4.2xlarge"
  subnet_id       = aws_subnet.Management.id
  private_ip      = var.aws_mgmt_pano
  security_groups = [aws_security_group.test.id]
  tags = {
    Name = var.h-panoramaname
  }
  key_name = "awsauto"
}

resource "aws_eip" "panoramapub" {
  vpc      = true
  instance = aws_instance.awsha_panorama.id
